...
class PriorityQueue {
  ...
  public void put(int key, Object el) {
    ...
    int j = i/2;
    data.add(null);
    KeyElement parent = (KeyElement) data.elementAt(j);
    while (parent.getKey() > key) {
      data.setElementAt(parent, i);
      i = j;
      j = j/2;
      parent = (KeyElement) data.elementAt(j);
    }
  ...
  public int minKey() throws ArrayIndexOutOfBoundsException {
    return ((KeyElement) data.elementAt(1)).getKey();
  }
  ...
  public Object removeMin() throws ArrayIndexOutOfBoundsException {
    Object min = ((KeyElement) data.elementAt(1)).getElement();
    KeyElement last = (KeyElement) data.remove(data.size() - 1);
    int key = last.getKey();
    int i = 1;
    int j = 2;
    while (j < data.size()) {
      if (j+1 < data.size() &&
	  ((KeyElement) data.elementAt(j)).getKey() >
	  ((KeyElement) data.elementAt(j+1)).getKey()) j++;
      if (((KeyElement) data.elementAt(j)).getKey() > key) break;
